	...
	if ($connection) { // got the database handler and the database itself connected
		...
		if ($result) {
			if (db_num_rows($result) > 0) {
				...
				query('ALTER TABLE ' . $_zp_conf_vars['mysql_prefix'] . 'administrators' . ' ADD COLUMN valid integer default 1', false);
				query('ALTER TABLE ' . $_zp_conf_vars['mysql_prefix'] . 'administrators' . ' CHANGE "password" pass varchar(64)', false);
				query('ALTER TABLE ' . $_zp_conf_vars['mysql_prefix'] . 'administrators' . ' ADD COLUMN loggedin timestamp', false);
				query('ALTER TABLE ' . $_zp_conf_vars['mysql_prefix'] . 'administrators' . ' ADD COLUMN lastloggedin timestamp', false);
