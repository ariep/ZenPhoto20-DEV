...
/**
 * Database core functions for the PDO::pgsql library
 *
...
define('DATABASE_SOFTWARE', 'PDO::pgsql');
Define('DATABASE_MIN_VERSION', '9.0');
Define('DATABASE_DESIRED_VERSION', '9.4');
...
function db_connect($config, $errorstop = true) {
	...
	try {
		...
		if (class_exists('PDO')) {
			$_zp_DB_connection = new PDO("pgsql:host=$hostname;dbname=$db;user=$username;password=$password");
		}
	} catch (PDOException $e) {
		$_zp_DB_last_result = $e;
		if ($errorstop) {
			zp_error(sprintf(gettext('PostgreSQL Error: ZenPhoto20 received the error %s when connecting to the database server.'), $e->getMessage()));
		}
